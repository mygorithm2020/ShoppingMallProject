function go_next() {
	if (document.formm.okon1[0].checked == true) {   
		document.formm.action = "market.do?command=join_form";
...
function customer_id() {
	document.formm.action = "market.do?command=login";
	document.formm.submit();	
}


function admin_id() {
	document.formm.action = "market.do?command=admin";
	document.formm.submit();	
}
...
function idcheck() {
	...
	var url="market.do?command=Id_Check_Form&id=" + document.formm.id.value;
...
function go_save(){
	if(document.formm.id.value=="") {
		...
		document.formm.action = "market.do?command=join";
		document.formm.submit();
	}		
}
...
function go_update() {
	if(document.formm.pw.value=="") {
		...
		document.formm.action = "market.do?command=Member_Update";
		document.formm.submit();
	}	
}
...
function go_delete() {
	document.formm.action = "market.do?command=Member_Delete_Form";
	document.formm.submit();
}
...
function go_delete_last(){
	alert('회원탈퇴가 완료되었습니다.');
	document.formm.action = "market.do?command=Member_Delete";
	document.formm.submit();
}
...
/* function go_delete() {
	...
	var url="market.do?command=Member_Delete&id=" + document.formm.id.value;
	var opt = "toolbar=no, menubar=no, scrollbars=no, resizable=no, width=500, height=250";
